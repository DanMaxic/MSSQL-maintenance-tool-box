...
DECLARE @MODE TABLE  
      (
            [ALL]                   BIT DEFAULT (0), 
            WAIT                    BIT DEFAULT (0),
            MEMORY                        BIT DEFAULT (1),
            SUSPENDED               BIT DEFAULT (0),
            CPU                           BIT DEFAULT (1), 
            BLOCKING                BIT DEFAULT (0),
            ...
            [REPLICATION COMMANDS] BIT DEFAULT (0)
      )
...
END
IF EXISTS (SELECT 1 FROM @MODE WHERE [ALL] = 1 OR MEMORY = 1)
BEGIN
      select '***************Performance Server Memory Values***************' as QueryMode
      ;with 
            cte_perf as
      (
            select * from sys.dm_os_performance_counters c where object_name = 'SqlServer:Buffer Manager' and cntr_type=65792
      )
      , 
            cte_pivot as
      (
            select * 
            from  cte_perf
            pivot(max(cntr_value) for counter_name in ([Total pages],[Target pages],[Free pages],[Stolen Pages],[Page life expectancy sec],[Reserved pages],[Database Pages])) as P
      )
      select * from cte_pivot
      
      select '***************Performance Server Memory Values MB***************' as QueryMode
      ;with 
            cte_perf as
      (
            select * from sys.dm_os_performance_counters c where object_name = 'SqlServer:Buffer Manager' and cntr_type=65792
      ...
      select [Total pages]*8/1024/1024 as [Total GB Of Memory By Server] from cte_pivot
      
      select '***************Server Memory By Databases***************' as QueryMode
    ...
    ,CASE database_id 
      ...
      GROUP BY db_name(database_id) ,database_id order by cached_MB desc
      select '***************Database Memory By Objects***************' as QueryMode
      
                  declare @dbname sysname
                  set @dbname =
                  (
                        SELECT top 1 db_name(database_id) from
                        (
                                    SELECT count(*)*8/1024 AS cached_MB,database_id
                                    FROM sys.dm_os_buffer_descriptors
                                    GROUP BY db_name(database_id) ,database_id 
                        )T Order by cached_MB desc
                  )
...
END
IF EXISTS (SELECT 1 FROM @MODE WHERE [ALL] = 1 OR [REPLICATION COMMANDS] = 1)
BEGIN

select '***************REPLICATION COMMANDS***************' as QueryMode




/*** Transactions/Commands flow in distribution DataBase ***/


SELECT TOP 100
         CAST(B.command AS NVARCHAR(4000)) AS Commands
        ,A.entry_time
        ,A.xact_id
        ,A.xact_seqno
        ,B.article_id
        ,B.command_id
FROM     distribution.dbo.MSrepl_transactions AS A
         INNER JOIN distribution.dbo.MSrepl_commands AS B
            ON A.xact_seqno = B.xact_seqno
               AND A.publisher_database_id = B.publisher_database_id
WHERE    A.entry_time > DATEADD(mi , -30 , GETDATE())
